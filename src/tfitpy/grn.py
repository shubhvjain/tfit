...
def target_overlap_score(
    ...
    """
    if grn_graph is None:
        edges = get_collecTRI_Human(config)
        grn_graph = get_grn_graph(edges)
    ...
    for reg1, reg2 in pairs:
        ...
        row = {
            "tf1": info["reg1"],      # match hypergeom_index_score naming
            "tf2": info["reg2"],
            "target_overlap_score": score,
            "N1": info["N1"],  # targets of tf1
            "N2": info["N2"],  # targets of tf2
            "c": info["c"],    # common targets
            "pvalue": info["pvalue"]
        }
