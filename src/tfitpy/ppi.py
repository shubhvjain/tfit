from tfitpy.data import biogrid, hippie, stringdb
...
def get_ppi(config, source="hippie"):
    """
    Retrieve PPI network from specified database.
    
    :param config: Configuration for database access
    :param source: PPI database name ("hippie", "stringdb", "biogrid")
    :return: NetworkX graph of PPI network
    """
    db = {
        "hippie": hippie.get_ppi,
        "stringdb": stringdb.get,
        "biogrid": biogrid.get
    }
    if source in db:
        data= db[source](config=config)
        return get_ppi_networkx(data)
    else:
        raise ValueError(f"Invalid source name: {source}. Choose from {list(db.keys())}")
...
def shortest_path_score(
        config, module: Dict[str, Any], 
        ppi_source: str = "hippie", 
    ...
    if ppi_network is None:
        ppi_network = get_ppi(config, ppi_source)
    
    print(ppi_network)

    pairs = generate_tf_pairs(module["gene_cluster"])
    pair_results = []
...
def hypergeom_index_score(
    ...
    valid_scores = pairs_df["hypergeom_score"].replace([np.inf, -np.inf], np.nan)
